...
class BehaviorScenario:
    ...
    def __init__(self, behavior_name: str):
        ...
        self.current_scenario = self.scenarios.get(behavior_name, self.scenarios["wake_up"])
...
def behavior_to_movement(scenario: BehaviorScenario, step: int, fps: int) -> float:
    ...
    if scenario.phase < scenario.total_phases - 1:
        ...
        transition_factor = min(1.0, phase_progress * 2)  # Transition sur la dernière moitié
        base_movement = base_movement * (1 - transition_factor) + next_movement(0) * transition_factor
...
def main():
    ...
    parser.add_argument("--behavior", default="wake_up", help="Comportement BBIA (wake_up, greeting, emotional_response)")
    parser.add_argument("--duration", type=int, default=10, help="Durée totale en secondes")
    parser.add_argument("--headless", action="store_true", help="Mode headless")
    parser.add_argument("--joint", default="yaw_body", help="Joint à animer")
    parser.add_argument("--intensity", type=float, default=1.0, help="Intensité du comportement")
    ...
    print(f"✅ BBIA Comportements initialisés")
    ...
    if args.headless:
        ...
        for step in range(total_steps):
            ...
            if step % 20 == 0:
                ...
                current_phase = scenario.get_current_phase()
                print(f"  Step {step:3d} | t={elapsed:3.1f}s | phase={current_phase['phase']} | {args.joint}={angle:6.3f} rad")
