...
def lister_voix_disponibles():
    ...
    for idx, v in enumerate(voices):
        try:
            langue = v.languages[0].decode(errors='ignore') if hasattr(v.languages[0], 'decode') else str(v.languages[0])
        except Exception:
            langue = str(v.languages)
        print(f"{idx+1}. Nom : {v.name} | Langue : {langue} | Genre : {getattr(v, 'gender', '?')} | ID : {v.id}")
