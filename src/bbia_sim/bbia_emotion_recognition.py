...
try:
    ...
    ML_AVAILABLE = False
    logger.warning("Dépendances ML non disponibles. Installez avec: pip install mediapipe torch transformers")
...
class BBIAEmotionRecognition:
    ...
    def __init__(self, device: str = "auto"):
        ...
        """
        if not ML_AVAILABLE:
            raise ImportError("Dépendances ML requises. Installez avec: pip install mediapipe torch transformers")
    ...
    def analyze_facial_emotion(self, image: Union[np.ndarray, str], face_bbox: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    def analyze_vocal_emotion(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            emotion_result = self.emotion_models["emotion"](text)
            
            # Analyse de sentiment
            sentiment_result = self.emotion_models["sentiment"](text)
            ...
            detected_emotion = emotion_mapping.get(emotion_result[0]["label"], "neutral")
    ...
    def fuse_emotions(self, facial_result: Dict[str, Any], vocal_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def analyze_emotion_realtime(self, image: Union[np.ndarray, str], text: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _update_emotion_history(self, emotion_result: Dict[str, Any]):
        ...
        max_history = self.detection_config["temporal_window_size"] * 2
    ...
    def _apply_temporal_smoothing(self, current_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_emotions = self.emotion_history[-self.detection_config["temporal_window_size"]:]
            ...
            if emotion_counts:
                ...
                smoothed_confidence = emotion_counts[smoothed_emotion] / len(recent_emotions)
    ...
    def get_emotion_statistics(self) -> Dict[str, Any]:
        ...
        # Confiance moyenne
        avg_confidence = sum(r.get("confidence", 0) for r in self.emotion_history) / len(self.emotion_history)
...
def main():
    ...
    vocal_result = emotion_rec.analyze_vocal_emotion("Je suis très heureux aujourd'hui!")
    print(f"Résultat: {vocal_result}")
    
    # Test analyse émotion faciale (simulation)
    print("\n😊 Test analyse émotion faciale...")
    facial_result = emotion_rec.analyze_facial_emotion(np.random.randint(0, 255, (480, 640, 3), dtype=np.uint8))
