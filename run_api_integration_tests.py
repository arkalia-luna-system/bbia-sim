#!/usr/bin/env python3
"""Script de test pour les tests d'intégration d'API."""
...
def main():
    """Fonction principale."""
    print("🧪 Exécution des tests d'intégration d'API")
    ...
    # Exécuter les tests d'intégration d'API
    print("\n🌐 Tests d'intégration d'API")
    success = run_command([
        "python", "-m", "pytest", "-v", "--cov=src/bbia_sim", 
        "--cov-report=term-missing", "--cov-report=xml",
        "--cov-report=html", "-m", "not slow"
    ] + api_integration_tests, "Tests d'intégration d'API avec couverture")
    
    # Résumé
    print("\n📊 Résumé")
    print(f"Tests d'intégration d'API: {'✅' if success else '❌'}")
    
    if success:
        print("\n🎉 Les tests d'intégration d'API passent!")
        print("📁 Rapport de couverture HTML généré dans htmlcov/")
        print("📄 Rapport de couverture XML généré dans coverage.xml")
        sys.exit(0)
    else:
        print("\n💥 Les tests d'intégration d'API ont échoué")
        sys.exit(1)
