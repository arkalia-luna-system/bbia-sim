#!/usr/bin/env python3
"""
Veille Reachy Mini — recherche intelligente + suivi officiel + notifications.

Fonctions clés:
- GitHub/Hugging Face: détection candidats via mots-clés Reachy Mini (filtrage strict)
- Filtrage temporel GitHub (pushed:>= dernière exécution) pour éviter les recherches dans le vide
- Scoring enrichi (API/WS, CI/coverage, dashboard, simu, émotions, tests, topics)
- Export CSV + JSON + DIFF JSON
- Suivi officiel pollen-robotics/reachy_mini + parsing email OFFICIAL_NEWS_FILE (shipping.*)
- Options anti rate-limit (pause, stop-on-limit) et affichage "seulement changements"
- Notification macOS (osascript) en cas de changements si VEILLE_NOTIFY_MACOS=true
"""
...
try:
    ...
    print("[veille] Module 'requests' manquant. Installez requirements/requirements-veille.txt.", file=sys.stderr)
    raise
    ...
    for kw in keywords:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                print("[veille][github] Rate limited — réduisez la fréquence ou définissez GH_TOKEN", file=sys.stderr)
                if stop_on_limit:
                    break
            else:
                print(f"[veille][github] HTTP {resp.status_code} pour '{kw}': {resp.text[:200]}", file=sys.stderr)
...
def github_fetch_repo_signals(repo: dict, gh_token: str | None) -> dict[str, bool]:
    ...
    try:
        t = session.get(f"{api_base}/topics", headers={**headers, "Accept": "application/vnd.github.mercy-preview+json"}, timeout=20)
    ...
    if txt:
        ...
        if "dashboard" in txt or "web ui" in txt or "frontend" in txt:
            has_dashboard = True
        if "emotion" in txt or "affective" in txt or "mood" in txt or "cognitive" in txt:
            has_emotion = True
    ...
    try:
        ...
        if tree.status_code == 200 and isinstance(tree.json(), list):
            ...
            if any(n.startswith("dockerfile") or n == "docker-compose.yml" for n in names):
                has_docker = True
    ...
    return {
        ...
        "tests": has_tests,
        "docker": has_docker,
        "topic_reachy_mini": any("reachy" in t.lower() and "mini" in t.lower() for t in topics),
    }
...
def search_github_candidates(keywords: list[str], gh_token: str | None, since_date: str | None) -> list[Candidate]:
    ...
    for r in repos:
        ...
        plausible = _is_reachy_mini_text(text) or signals.get("topic_reachy_mini", False)
...
def write_diff_json(path: str, prev: dict[str, dict[str, Any]], curr: list[Candidate]) -> None:
    now = dt.datetime.utcnow().isoformat()
    ...
    for i in prev_keys & curr_keys:
        ...
        if prev_score != curr_score:
            changed.append({"identifier": i, "prev_score": prev_score, "curr_score": curr_score})
    payload = {"timestamp": now, "new": [curr_map[i] for i in new_ids], "removed": [prev[i] for i in removed_ids], "changed_scores": changed}
    with open(path, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)


def notify_macos(title: str, message: str) -> None:
    if os.environ.get("VEILLE_NOTIFY_MACOS", "false").lower() not in {"1", "true", "yes"}:
        return
    try:
        subprocess.run([
            "osascript",
...
def main(argv: list[str]) -> int:
    ...
    exclude_official = os.environ.get("VEILLE_EXCLUDE_OFFICIAL", "false").lower() in {"1", "true", "yes"}
    ...
    if exclude_official:
        def _is_official(c: Candidate) -> bool:
            return c.identifier.startswith("https://github.com/pollen-robotics/") or c.identifier.startswith("https://huggingface.co/spaces/pollen-robotics/")
    ...
    if changes_new or changes_removed or changes_scores:
        notify_macos("Veille Reachy Mini", f"Nouveaux:{changes_new} Retirés:{changes_removed} Scores:{changes_scores}")
    ...
    if watch_official:
        try:
            ...
            if official_news_file and os.path.exists(official_news_file):
                try:
                    ...
                    if m_next:
                        ...
                        year = now.year + (1 if now.month > 11 else 0)
                        status.setdefault("shipping", {})
            ...
            if changed_fields:
                ...
                notify_macos("Veille Reachy Mini — Officiel", f"{len(changed_fields)} changement(s) détecté(s)")
        except Exception as exc:  # pragma: no cover
