    ...
    if "12 émotions" not in content.lower() and "douze émotions" not in content.lower():
        if "emotion" in content.lower() and "bbia" in content.lower():
            ...
            if emotion_match and emotion_match.group(1) != "12":
                issues.append(f"Nombre d'émotions incorrect: {emotion_match.group(1)} (devrait être 12)")
...
def improve_markdown_style(content: str) -> str:
    ...
    while i < len(lines):
        ...
        if line.strip().startswith("```") and not line.strip()[3:].strip():
            ...
            while j < len(lines) and not lines[j].strip().startswith("```"):
                if lines[j].strip().startswith(("pip", "python", "import", "from", "def ", "class ")):
    ...
    content = re.sub(
        r"octobre\s+2025|Octobre\s+2025",
        "Octobre 2025",
        content
    )
    content = re.sub(
        r"(novembre|Novembre)\s+2025",
        "Oct 25 / Nov 25",
        content
    )
