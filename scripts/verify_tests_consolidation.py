    ...
    try:
        ...
        if result.returncode == 0:
            ...
            for line in result.stdout.split('\n'):
                ...
                if ('test_' in line and ('Function' in line or 'Method' in line)) or ('::' in line and 'test_' in line):
...
def generate_consolidation_report(all_tests: dict, duplicates: dict, unique_by_file: dict):
    ...
    files_with_unique = {
        f: len(t) for f, t in unique_by_file.items() if len(t) > 0
    }
    ...
    files_with_few_unique = {
        f: len(t) for f, t in unique_by_file.items() 
    ...
    if files_with_few_unique:
        ...
        for filepath, count in sorted(files_with_few_unique.items(), key=lambda x: -len(all_tests[x[0]])):
...
def main():
    ...
    if not all_tests:
        print("❌ Aucun test collecté. Vérifiez que pytest est installé et que les fichiers existent.")
        sys.exit(1)
    ...
    if total_unique == total_before:
        print("   ✅ Aucun doublon détecté (ou tous les tests sont dans un seul fichier)")
    else:
