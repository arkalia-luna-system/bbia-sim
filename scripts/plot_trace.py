...
def plot_joint_movements(trace_data: List[Dict[str, Any]], 
                        output_file: str, 
                        title: str = "Mouvements joints BBIA") -> bool:
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"  - Fréquence moyenne: {len(trace_data) / (max(times) - min(times)):.1f} Hz\n\n")
        ...
        for joint in unique_joints:
            ...
            if joint_positions:
                ...
                if max_pos > 0.3 or min_pos < -0.3:
                    f.write(f"  ⚠️  ATTENTION: Limite sécurité dépassée!\n")
                else:
                    f.write(f"  ✅ Limites sécurité respectées\n")
                f.write("\n")
...
def plot_latency_analysis(trace_data: List[Dict[str, Any]], 
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"📊 STATISTIQUES TEMPORELLES:\n")
        f.write(f"  - Nombre de mesures: {len(trace_data)}\n")
        ...
        f.write(f"🎯 ANALYSE DE STABILITÉ:\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Génère des graphiques à partir des traces BBIA")
    ...
    parser.add_argument("--title", default="Mouvements joints BBIA", help="Titre du graphique")
    parser.add_argument("--type", choices=["movements", "latency"], default="movements", 
