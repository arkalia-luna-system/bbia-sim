...
class ProcessManager:
    ...
    def start_process(self, mode: str = "graphical", duration: Optional[int] = None) -> bool:
        """Démarre un processus BBIA avec vérification de doublons."""
        
        # Vérifier les doublons
        if self.is_locked():
            logger.error("❌ Un processus BBIA est déjà en cours d'exécution!")
            logger.error("💡 Utilisez 'python scripts/process_manager.py stop' pour l'arrêter")
            return False
    ...
    def show_status(self) -> None:
        ...
        if running:
            ...
            for proc in running:
                ...
                logger.info(f"   {status} PID {proc['pid']}: {proc['cpu_percent']:.1f}% CPU, {proc['memory_mb']:.1f}MB RAM")
...
def main():
    ...
    parser.add_argument("action", choices=["start", "stop", "status", "kill-all"], 
                       help="Action à effectuer")
    parser.add_argument("--mode", choices=["graphical", "headless", "test"], 
                       default="graphical", help="Mode de démarrage")
    parser.add_argument("--duration", type=int, help="Durée en secondes (headless uniquement)")
    parser.add_argument("--force", action="store_true", help="Mode force (pas de confirmation)")
    ...
    if args.action == "start":
        ...
        if success:
            ...
            logger.info("💡 Utilisez 'python scripts/process_manager.py status' pour vérifier")
            logger.info("💡 Utilisez 'python scripts/process_manager.py stop' pour arrêter")
        else:
