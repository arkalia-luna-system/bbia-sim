...
class BBIADemo:
    ...
    async def demo_ecosystem_info(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"   • Mode simulation: {data.get('simulation_mode', 'N/A')}")
                
                emotions = data.get('emotions', [])
                logger.info(f"   • Émotions ({len(emotions)}): {', '.join(emotions[:5])}{'...' if len(emotions) > 5 else ''}")
                
                behaviors = data.get('behaviors', [])
                logger.info(f"   • Comportements ({len(behaviors)}): {', '.join(behaviors[:5])}{'...' if len(behaviors) > 5 else ''}")
            ...
            for emotion in test_emotions:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(f"      Joints affectés: {len(data.get('joints_affected', []))}")
                    else:
                        logger.warning(f"   ⚠️ {emotion}: Échec (code {response.status_code})")
            ...
            for behavior in test_behaviors:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(f"      Durée estimée: {data.get('estimated_duration', 'N/A')}s")
                    else:
                        logger.warning(f"   ⚠️ {behavior}: Échec (code {response.status_code})")
    ...
    async def demo_documentation(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"📊 Spécification OpenAPI:")
                ...
                logger.info(f"   • Exemples d'endpoints:")
            ...
            logger.info(f"   • GitHub: https://github.com/arkalia-luna-system/bbia-sim")
    ...
    async def run_complete_demo(self):
        ...
        logger.info("="*80)
        logger.info("✅ Toutes les fonctionnalités de l'API publique ont été démontrées")
