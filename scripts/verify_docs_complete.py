...
class DocsVerifier:
        ...
        for match in re.finditer(link_pattern, content):
            ...
            try:
                ...
                possible_paths = [
                    ...
                    (
                        PROJECT_ROOT / link_path.relative_to("/")
    ...
    def check_mermaid(self, md_file: Path, content: str) -> None:
        ...
        for match in re.finditer(mermaid_pattern, content, re.DOTALL):
            ...
            valid_type = any(
                diagram.startswith(f"{mtype}") or diagram.startswith(f"{mtype} ")
            ...
            if "style" not in diagram.lower() and any(
                t in diagram.lower()
    ...
    def check_formatting(self, md_file: Path, content: str) -> None:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if re.match(r"^[-*]\S", line) and not line.startswith("```"):
                ...
                if "://" in line or (
                    line.count("/") > 2 and not line.strip().startswith("-")
                ):
                    continue
    ...
    def check_dates(self, md_file: Path, content: str) -> None:
        ...
        old_dates = re.findall(
            r"\b(2024|2026|janvier|février|mars|mai|juin|juillet|août|septembre|décembre)\s+2025\b",
            content,
            re.IGNORECASE,
        )
        if old_dates:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        self.errors[md_file].append("❌ Erreur encodage fichier")
                        continue
...
def main() -> int:
    ...
    results = verifier.verify_all(
        ...
        code_consistency=args.code_consistency
        or (not args.links_only and not args.spell_only and not args.mermaid_only),
