...
def fix_multiline_fstring(lines: list[str], start_idx: int) -> tuple[int, list[str]]:
    ...
    logger_call_match = re.search(r'(logger\.(?:debug|info|warning|error|exception|critical))', full_call)
...
def fix_file(file_path: Path) -> int:
    ...
    try:
        ...
        while i < len(lines):
            ...
            if 'logger.' in line and ('f"' in line or "f'" in line) and line.count("(") > line.count(")"):
