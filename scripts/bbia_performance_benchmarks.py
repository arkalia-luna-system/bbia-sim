    ...
    def initialize_robot(self) -> bool:
        ...
        try:
            ...
            if self.robot:
                ...
                logger.info(f"Robot {self.backend} {'connecté' if connected else 'en simulation'}")
                return True
            return False
    ...
    def benchmark_load(self) -> Dict[str, Any]:
            ...
            for i in range(requests):
                try:
                    ...
                    if operation == 0:
                        ...
                        client_results["latencies"].append((joint_end - joint_start) * 1000)
                        ...
                        client_results["latencies"].append((emotion_end - emotion_start) * 1000)
                        ...
                        client_results["latencies"].append((telemetry_end - telemetry_start) * 1000)
        ...
        for num_clients in self.test_config["load_tests"]["concurrent_clients"]:
            ...
            with ThreadPoolExecutor(max_workers=num_clients) as executor:
                ...
                client_results = []
                for future in as_completed(futures, timeout=self.test_config["load_tests"]["timeout"]):
                    try:
            ...
            total_requests = sum(r["successful_requests"] + r["failed_requests"] for r in client_results)
    ...
    def generate_report(self, results: Dict[str, Any]) -> str:
        ...
        if "load" in results["benchmarks"]:
            ...
            for test_name, test_data in load_data["load_tests"].items():
                ...
                report.append(f"  Mean latency: {test_data['latency_stats']['mean_ms']:.3f} ms")
                report.append(f"  P95 latency: {test_data['latency_stats']['p95_ms']:.3f} ms")
                report.append("")
        ...
        if "memory" in results["benchmarks"]:
            ...
            for test_name, test_data in memory_data["memory_tests"].items():
                report.append(f"{test_name}:")
                report.append(f"  Memory growth: {test_data['memory_growth_mb']:.2f} MB")
                if "peak_memory_mb" in test_data:
                    report.append(f"  Peak memory: {test_data['peak_memory_mb']:.2f} MB")
                report.append("")
...
def main():
    ...
    try:
        ...
        print(f"✅ Benchmarks terminés avec succès")
