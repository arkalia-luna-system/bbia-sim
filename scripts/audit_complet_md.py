...
def check_dates_in_file(file_path: Path) -> list[dict[str, Any]]:
    ...
    try:
        ...
        date_patterns = [
            ...
            (r"(octobre|novembre|décembre|janvier|février|mars|avril|mai|juin|juillet|août|septembre)\s+(2024|2025|2026)", "month_year"),
            (r"\b(2024|2025|2026)\b", "year_only"),
            (r"(Oct|Nov|Dec|Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep)\s*25\s*/\s*(Nov|Oct|Dec)", "recent_format"),
        ]
        
        # Vérifier si c'est un fichier récent (novembre 2025)
        is_recent = "nov" in file_path.name.lower() or "nov2025" in file_path.name.lower() or file_path.name.startswith("VERIFICATION") or "NOV" in file_path.name
...
def categorize_file(file_path: Path, root_dir: Path) -> dict[str, Any]:
    ...
    # Fichiers à archiver
    temp_indicators = ["analyse", "audit", "verification", "resume", "recap", "activite", "bilan"]
    if any(ind in name for ind in temp_indicators) and rel_path.parts[0] not in ["docs", "presentation"]:
        should_archive = True
        is_temporary = True
        category = "temporary_analysis"
    
    # Fichiers redondants
    redundant_indicators = ["final", "complet", "complete", "comprehensive", "exhaustif"]
    ...
    if len(rel_path.parts) == 1:
        if name in ["readme.md", "projects.md", "changelog.md", "contributing.md", "code_of_conduct.md"]:
            category = "root_main"
        else:
            category = "root_other"
            should_archive = True
...
def main():
    ...
    print(f"3. Vérifier contenu vs code pour fichiers principaux")
