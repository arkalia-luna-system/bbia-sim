...
def check_functionality_implemented(name: str, info: dict[str, Any], root_dir: Path) -> bool:
        ...
        if todo_in_content:
            ...
            for i, line in enumerate(lines):
                if any(p in line for p in info["patterns"]):
                    ...
                    nearby = " ".join(lines[max(0, i-2):min(len(lines), i+3)]).upper()
                    if "TODO" in nearby:
...
def main():
        ...
        for todo in todos[:2]:  # 2 premiers par fichier
            ...
            for func_name, is_impl in implemented.items():
                if func_name.lower() in line_content.lower() and is_impl:
                    ...
                    print(f"  ⚠️  OUTDATÉ (ligne {todo['line']}): {func_name} est implémenté")
                    break
    ...
    print("="*60)
    print("\n📊 RÉSUMÉ\n")
    print(f"✅ Fonctionnalités implémentées: {sum(implemented.values())}/{len(implemented)}")
