...
def replace_dates_to_october_2025(content: str) -> str:
    ...
    content = re.sub(r'\b(20[0-9]{2})-([0-9]{2})-([0-9]{2})\b', replace_yyyy_mm_dd, content)
    
    # Remplace dates format DD/MM/YYYY
    content = re.sub(r'\b([0-9]{1,2})/([0-9]{1,2})/(20[0-9]{2})\b', replace_dd_mm_yyyy, content)
...
def process_md_files(root_dir: str):
    ...
    exclude_dirs = {'venv', 'venv-vision', 'venv-voice', 'venv-vision-py310', 
                    'dist', 'build', '__pycache__', '.git', 'htmlcov', 
                    'src/bbia_sim.egg-info', 'node_modules'}
