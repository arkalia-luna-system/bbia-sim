...
def compare_assets() -> None:
    ...
    official_audio = list((OFFICIAL_ROOT / "src" / "reachy_mini" / "assets").glob("*.wav"))
...
def compare_constants() -> None:
    """Compare constantes."""
    official_constants = OFFICIAL_ROOT / "src" / "reachy_mini" / "utils" / "constants.py"
    if official_constants.exists():
        with open(official_constants, "r") as f:
...
def compare_error_messages() -> None:
    ...
    for pattern in patterns:
        ...
        for router_file in (OFFICIAL_ROOT / "src" / "reachy_mini" / "daemon" / "app" / "routers").glob("*.py"):
            with open(router_file, "r") as f:
                content = f.read()
                if re.search(pattern, content):
                    official_matches.append(str(router_file))
        
        for router_file in (BBIA_ROOT / "src" / "bbia_sim" / "daemon" / "app" / "routers").glob("*.py"):
            with open(router_file, "r") as f:
                content = f.read()
                if re.search(pattern, content):
...
def compare_config_files() -> None:
    ...
    if official_pyproject.exists() and bbia_pyproject.exists():
        with open(official_pyproject, "r") as f:
            official_content = f.read()
        with open(bbia_pyproject, "r") as f:
            bbia_content = f.read()
