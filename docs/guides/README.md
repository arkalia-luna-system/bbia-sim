# 📚 Guides BBIA-SIM

Ce dossier contient tous les guides pratiques pour utiliser BBIA-SIM.

## 📁 Guides Disponibles

### 🚀 Démarrage
- **`GUIDE_DEMARRAGE.md`** - Guide de démarrage rapide (5 min)
- **`GUIDE_AVANCE.md`** - Guide technique (15 min)
- **`DEMARRAGE_DAEMON.md`** - Guide pour démarrer le daemon

### 💬 Intelligence & Conversation
- **`GUIDE_CHAT_BBIA.md`** - Guide du chat BBIA
- **`GUIDE_LLM_CONVERSATION.md`** - Guide LLM conversationnel
- **`GUIDE_NLP_SMOLVLM.md`** - Guide NLP avec SmolVLM

### 🤖 Comportements & Robot
- **`GUIDE_COMPORTEMENTS.md`** - Guide des comportements
- **`REACHY_MINI_WIRELESS_COMPLETE_GUIDE.md`** - Guide complet Reachy Mini Wireless

### 📹 Démonstrations & Partage
- **`GUIDE_VIDEOS_DEMONSTRATION.md`** - Guide des vidéos de démonstration
- **`PARTAGE_APPS_BBIA_HF_HUB.md`** - Partage d'apps sur Hugging Face Hub

## 📚 Documentation Associée

- **Installation** : [`../installation/`](../installation/)
- **Hardware** : [`../hardware/`](../hardware/)
- **Développement** : [`../development/`](../development/)

---

**Dernière mise à jour** : 8 Décembre 2025

